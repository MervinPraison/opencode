...
export interface Interface {
  ...
  readonly get: (url: string) => Effect.Effect<Entry | undefined>
  readonly set: (url: string, entry: Entry) => Effect.Effect<void>
  readonly remove: (url: string) => Effect.Effect<void>
  readonly metadata: (url: string) => Effect.Effect<Metadata>
  readonly configs: () => Effect.Effect<ConfigDocument[]>
}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const load: () => Effect.Effect<Record<string, Entry>> = Effect.fnUntraced(function* () {
      ...
      return yield* loadLegacyAuth({ fsys, dataDir: global.data, write: (data) => write("migrate", data) }).pipe(
        Effect.orDie,
      )
    })
    ...
    return Service.of({
      ...
      metadata: Effect.fn("AuthWellKnown.metadata.public")(function* (url) {
        return (yield* metadata(url).pipe(Effect.orDie)).metadata
      }),
